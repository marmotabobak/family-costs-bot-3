...
- запись расхода со спецсимволами, например: тест_2 1
- не вносится запись с большим количеством данных, например:
собирать URL сервисом самостоятельно (не импортом из env): данные доступны из других env 1
собирать URL сервисом самостоятельно (не импортом из env): данные доступны из других env 1
собирать URL сервисом самостоятельно (не импортом из env): данные доступны из других env 1
собирать URL сервисом самостоятельно (не импортом из env): данные доступны из других env 1
собирать URL сервисом самостоятельно (не импортом из env): данные доступны из других env 1
...
собирать URL сервисом самостоятельно (не импортом из env): данные доступны из других env 1
собирать URL сервисом самостоятельно (не импортом из env): данные доступны из других env 1
- аналогично: не выводит отчет за месяц с большим кол-вом данных (пример: сент, окт 2026)
- ИЛИ проблема с оффсетом?  TelegramBadRequest: Telegram server says - Bad Request: can't parse entities: Can't find end of the entity starting at byte offset 304
